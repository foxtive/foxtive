...
fn main() {
    ...
    println!("   Peak hours (9-17):    {} - every 5 min", peak_monitoring.build());
    println!("   Off-peak hours:       {} - every 30 min", offpeak_monitoring.build());
    println!();
    ...
    let hourly_cleanup = CronExpression::builder()
        .hourly()
        .minute(0);
    
    let daily_cleanup = CronExpression::builder()
        .daily()
        .hour(1);
    ...
    let monthly_cleanup = CronExpression::builder()
        .monthly()
        .day_of_month(1)
        .hour(3);
