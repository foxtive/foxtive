...
async fn main() -> anyhow::Result<()> {
    ...
    cron.add_job_fn("cache-cleanup", "Cache Cleanup", "0 0 0,6,12,18 * * * *", || async {
    ...
    cron.add_job_fn("biz-monitor", "Business Hours Monitor", "0 */15 9-17 * * 1-5 *", || async {
    ...
    cron.add_job_fn("monthly-report", "Monthly Report", "0 0 9 1 * * *", || async {
    ...
    cron.add_job_fn("rate-limit-reset", "Rate Limit Reset", "0 0 * * * * *", || async {
    ...
    cron.add_job_fn("security-scan", "Security Scan", "0 0 1 * * * *", || async {
    ...
    cron.add_job_fn("webhook-retry", "Webhook Retry Queue", "0 * * * * * *", || async {
