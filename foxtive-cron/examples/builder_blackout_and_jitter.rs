...
fn main() {
    ...
    let mut backup_schedule = CronExpression::builder().daily().hour(2).minute(30);
    ...
    let mut log_rotation = CronExpression::builder().daily().hour(3);
    ...
    println!(
        "   Critical (30s):     {} - ±2s jitter",
        critical_health.build()
    );
    println!(
        "   Standard (5min):    {} - ±30s jitter",
        standard_health.build()
    );
    println!(
        "   Summary (hourly):   {} - ±5min jitter",
        summary_health.build()
    );
