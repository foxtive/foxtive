...
use foxtive_worker::{Worker, ReceivedMessage, MessageBackend};
...
impl DlqMonitorWorker {
    ...
    async fn send_alert(&self, dlq_msg: &DeadLetterMessage) {
        ...
        println!("     Subject: Poison Pill Detected - {}", dlq_msg.original_id);
        println!("     Body: Message has failed {} times in source queue '{}'", 
...
async fn main() -> anyhow::Result<()> {
    ...
    {
        println!("RabbitMQ feature not enabled. Run with: cargo run --example dlq_consumer --features rabbitmq");
        return Ok(());
    }
    ...
    #[cfg(feature = "rabbitmq")]
    {
        ...
        let backend = Arc::new(RabbitMqBackend::new("amqp://ahmard:Pass.1234@localhost", dlq_config).await?);
