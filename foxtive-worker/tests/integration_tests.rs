...
async fn test_rapid_message_burst() {
    ...
    for i in 0..100 {
        pool.dispatch(create_test_message(&format!("burst-{:03}", i))).await.unwrap();
    }
    let dispatch_time = start.elapsed();
    
    // Dispatch should be fast (non-blocking)
    assert!(dispatch_time < Duration::from_secs(1), "Dispatch should be non-blocking");
    
    // Wait for all to complete
    wait_for_condition(
        || worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 100,
        Duration::from_secs(10)
    ).await;
    
    assert_eq!(worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 100);
}
...
async fn test_worker_failure_handling() {
    ...
    for i in 0..10 {
        pool.dispatch(create_test_message(&format!("fail-msg-{}", i))).await.unwrap();
    }
    ...
    assert_eq!(processed, 10, "Worker should attempt all messages even if failing");
}
...
async fn test_round_robin_distribution() {
    ...
    wait_for_condition(
        || {
            let total = worker1.process_count.load(std::sync::atomic::Ordering::SeqCst)
    ...
    let count1 = worker1.process_count.load(std::sync::atomic::Ordering::SeqCst);
    let count2 = worker2.process_count.load(std::sync::atomic::Ordering::SeqCst);
    let count3 = worker3.process_count.load(std::sync::atomic::Ordering::SeqCst);
    
    // Each worker should have processed approximately 10 messages
    assert!((8..=12).contains(&count1), "Worker1 got {}, expected ~10", count1);
    assert!((8..=12).contains(&count2), "Worker2 got {}, expected ~10", count2);
    assert!((8..=12).contains(&count3), "Worker3 got {}, expected ~10", count3);
}
...
async fn test_least_loaded_balancing() {
    ...
    assert!(count1 > 0 && count2 > 0, "Both workers should process some messages");
}

/// Test pool shutdown prevents new dispatches.
#[tokio::test]
async fn test_pool_shutdown() {
    let worker = Arc::new(TestWorker::new("shutdown-worker").with_delay(Duration::from_millis(100)));
    ...
    assert!(result.is_err() || result.unwrap().is_err(), "Should not dispatch after shutdown");
}

/// Test empty pool rejects messages.
#[tokio::test]
async fn test_empty_pool_rejection() {
    let pool = WorkerPool::new("empty-pool", LoadBalancingStrategy::RoundRobin, Arc::new(NoOpMetrics));
    ...
    if let Err(e) = result {
        assert!(e.to_string().contains("exhausted") || e.to_string().contains("empty"),
                "Error should indicate pool is exhausted: {}", e);
    }
}
...
async fn test_concurrent_pool_access() {
    let worker = Arc::new(TestWorker::new("concurrent-worker").with_delay(Duration::from_millis(10)));
...
async fn test_background_job_processor() {
    ...
    for i in 0..50 {
        pool.dispatch(create_test_message(&format!("job-{:03}", i))).await.unwrap();
    }
    
    // Wait for completion
    wait_for_condition(
        || {
            let total = fast_worker.process_count.load(std::sync::atomic::Ordering::SeqCst)
                + slow_worker.process_count.load(std::sync::atomic::Ordering::SeqCst);
            total >= 50
        },
        Duration::from_secs(10)
    ).await;
    
    let fast_count = fast_worker.process_count.load(std::sync::atomic::Ordering::SeqCst);
    let slow_count = slow_worker.process_count.load(std::sync::atomic::Ordering::SeqCst);
    
    assert_eq!(fast_count + slow_count, 50, "All jobs should be processed");
    // Both workers should have processed some messages
    assert!(fast_count > 0 && slow_count > 0, "Both workers should handle jobs");
}

/// Test real-world scenario: Notification system with high throughput.
#[tokio::test]
async fn test_notification_system_high_throughput() {
    let notification_worker = Arc::new(
        TestWorker::new("notification-sender").with_delay(Duration::from_millis(5))
    );
    ...
    let start = std::time::Instant::now();
    for i in 0..200 {
        pool.dispatch(create_test_message(&format!("notif-{:04}", i))).await.unwrap();
    }
    
    // Wait for all to complete
    wait_for_condition(
        || notification_worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 200,
        Duration::from_secs(10)
    ).await;
    let elapsed = start.elapsed();
    
    assert_eq!(notification_worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 200);
    
    // With 50 concurrent and 5ms per notification, should complete in ~20ms + overhead
    assert!(elapsed < Duration::from_secs(2), "High throughput should be fast: {:?}", elapsed);
}
...
async fn test_large_message_volume() {
    ...
    for i in 0..500 {
        pool.dispatch(create_test_message(&format!("vol-{:04}", i))).await.unwrap();
    }
    
    // Wait for completion
    wait_for_condition(
        || worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 500,
        Duration::from_secs(30)
    ).await;
    
    assert_eq!(worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 500);
}
