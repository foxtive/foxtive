...
use foxtive_worker::{WorkerPool, WorkerPoolBuilder, LoadBalancingStrategy};
...
async fn test_concurrency_limit_strict_enforcement() {
    ...
    let max_concurrent = worker.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    assert!(
        max_concurrent <= 3,
    ...
    let processed = worker.process_count.load(std::sync::atomic::Ordering::SeqCst);
...
async fn test_independent_pool_limits() {
    ...
    let mut pool1 = WorkerPool::with_concurrency("pool1", LoadBalancingStrategy::RoundRobin, 2, Arc::new(NoOpMetrics));
    pool1.add_worker(worker1.clone());
    
    let mut pool2 = WorkerPool::with_concurrency("pool2", LoadBalancingStrategy::RoundRobin, 5, Arc::new(NoOpMetrics));
    pool2.add_worker(worker2.clone());
    
    // Dispatch messages to both pools
    for i in 0..10 {
        pool1.dispatch(create_test_message(&format!("p1-msg-{:02}", i))).await.unwrap();
        pool2.dispatch(create_test_message(&format!("p2-msg-{:02}", i))).await.unwrap();
    }
    ...
    let max1 = worker1.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    let max2 = worker2.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    ...
    assert_eq!(worker1.process_count.load(std::sync::atomic::Ordering::SeqCst), 10);
    assert_eq!(worker2.process_count.load(std::sync::atomic::Ordering::SeqCst), 10);
}
...
async fn test_builder_concurrency_limit() {
    ...
    for i in 0..15 {
        pool.dispatch(create_test_message(&format!("msg-{:02}", i))).await.unwrap();
    }
    ...
    assert_eq!(worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 15);
}

/// Test that concurrency limit of 1 enforces sequential processing.
#[tokio::test]
async fn test_sequential_processing_with_limit_one() {
    let worker = Arc::new(TestWorker::new("sequential-worker").with_delay(Duration::from_millis(20)));
    ...
    // Dispatch 5 messages
    for i in 0..5 {
        pool.dispatch(create_test_message(&format!("msg-{}", i))).await.unwrap();
    }
    ...
    let max_concurrent = worker.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    assert_eq!(max_concurrent, 1, "Sequential processing violated: max concurrent was {}", max_concurrent);
    
    assert_eq!(worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 5);
}
...
async fn test_zero_concurrency_limit() {
    ...
    assert!(result.is_err(), "Message should not be dispatched with concurrency limit 0");
}

/// Test high concurrency limit allows parallel processing.
#[tokio::test]
async fn test_high_concurrency_allows_parallelism() {
    let worker = Arc::new(TestWorker::new("high-concurrency-worker").with_delay(Duration::from_millis(100)));
    ...
    let current = worker.concurrent_count.load(std::sync::atomic::Ordering::SeqCst);
    assert!(
        current >= 5,
    ...
    let max_concurrent = worker.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    assert!(max_concurrent >= 10, "Expected at least 10 concurrent with high limit, got {}", max_concurrent);
    assert_eq!(worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 20);
}
...
async fn test_concurrency_across_multiple_workers() {
    ...
    let _total_max = worker1.max_concurrent.load(std::sync::atomic::Ordering::SeqCst)
        + worker2.max_concurrent.load(std::sync::atomic::Ordering::SeqCst)
        + worker3.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    
    // Note: Each worker tracks its own max, so we check individual workers
    assert!(worker1.max_concurrent.load(std::sync::atomic::Ordering::SeqCst) <= 4);
    assert!(worker2.max_concurrent.load(std::sync::atomic::Ordering::SeqCst) <= 4);
    assert!(worker3.max_concurrent.load(std::sync::atomic::Ordering::SeqCst) <= 4);
    
    // All messages should be distributed and processed
    let total_processed = worker1.process_count.load(std::sync::atomic::Ordering::SeqCst)
        + worker2.process_count.load(std::sync::atomic::Ordering::SeqCst)
        + worker3.process_count.load(std::sync::atomic::Ordering::SeqCst);
...
async fn test_payment_processor_scenario() {
    ...
    let payment_worker = Arc::new(
        TestWorker::new("payment-processor")
            .with_delay(Duration::from_millis(100)) // Payments take time
    );
    ...
    for i in 0..50 {
        pool.dispatch(create_test_message(&format!("payment-{:03}", i))).await.unwrap();
    }
    
    // Monitor that concurrency stays within bounds
    tokio::time::sleep(Duration::from_millis(500)).await;
    
    let max_concurrent = payment_worker.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    assert!(
        max_concurrent <= 10,
    ...
    wait_for_condition(
        || payment_worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 50,
        Duration::from_secs(10)
    ).await;
    
    assert_eq!(payment_worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 50);
}
...
async fn test_email_sender_scenario() {
    ...
    // Send 10 emails
    for i in 0..10 {
        pool.dispatch(create_test_message(&format!("email-{:02}", i))).await.unwrap();
    }
    
    // Wait for all to complete
    wait_for_condition(
        || email_worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 10,
        Duration::from_secs(5)
    ).await;
    
    // Verify all were processed
    assert_eq!(email_worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 10);
    
    // Verify concurrency never exceeded limit
    let max_concurrent = email_worker.max_concurrent.load(std::sync::atomic::Ordering::SeqCst);
    assert!(max_concurrent <= 2, "Email concurrency exceeded limit: {} > 2", max_concurrent);
}
