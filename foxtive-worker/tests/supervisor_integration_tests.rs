...
use foxtive_supervisor::enums::{RestartPolicy, BackoffStrategy};
...
async fn test_supervisor_restart_on_failure() {
    ...
    let supervisor = Supervisor::new()
        .add(FailingPool { restarts: restart_clone });
    ...
    assert!(restarts >= 2, "Expected at least 2 restarts, got {}", restarts);
    assert!(restarts <= 6, "Expected at most 6 restarts, got {}", restarts);
...
async fn test_graceful_shutdown_cascade() {
    let worker = Arc::new(
        TestWorker::new("shutdown-test-worker")
    ...
    assert!(shutdown_duration < Duration::from_secs(5), "Shutdown took too long: {:?}", shutdown_duration);
}

/// Test supervisor with worker pool that has middleware.
#[tokio::test]
async fn test_supervisor_with_middleware_pool() {
    use foxtive_worker::{TracingMiddleware, AckNackMiddleware};
...
async fn test_microservice_scenario() {
    ...
    let email_worker = Arc::new(TestWorker::new("email-worker").with_delay(Duration::from_millis(50)));
    let payment_worker = Arc::new(TestWorker::new("payment-worker").with_delay(Duration::from_millis(100)));
    let notification_worker = Arc::new(TestWorker::new("notification-worker").with_delay(Duration::from_millis(30)));
    ...
    let supervised_notification = SupervisedWorkerPool::new("notification-service", notification_pool);
...
async fn test_supervisor_recovers_from_panic() {
    ...
    let supervisor = Supervisor::new()
        .add(PanickingPool { panics: panic_clone });
...
async fn test_priority_based_ordering() {
    ...
    let low_priority = PriorityPool { name: "low-priority", priority: 1 };
    let high_priority = PriorityPool { name: "high-priority", priority: 10 };
    let medium_priority = PriorityPool { name: "medium-priority", priority: 5 };
...
async fn test_global_concurrency_limit() {
    let worker1 = Arc::new(TestWorker::new("concurrent-worker-1").with_delay(Duration::from_millis(100)));
    let worker2 = Arc::new(TestWorker::new("concurrent-worker-2").with_delay(Duration::from_millis(100)));
    let worker3 = Arc::new(TestWorker::new("concurrent-worker-3").with_delay(Duration::from_millis(100)));
