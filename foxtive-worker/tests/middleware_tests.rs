...
use foxtive_worker::{
    WorkerPoolBuilder,
    AckNackMiddleware, TracingMiddleware, CircuitBreakerMiddleware,
};
...
async fn test_middleware_chain_with_pool() {
    let worker = Arc::new(TestWorker::new("middleware-worker").with_delay(Duration::from_millis(10)));
    ...
    // Wait for completion
    wait_for_condition(
        || worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 10,
        Duration::from_secs(5)
    ).await;
    
    assert_eq!(worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 10);
}
...
async fn test_ack_nack_middleware_auto_ack() {
    ...
    // Wait for processing
    wait_for_condition(
        || worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 1,
        Duration::from_secs(2)
    ).await;
    
    // Message should be auto-acked on success
    assert!(acked.load(std::sync::atomic::Ordering::SeqCst), "Message should be auto-acked");
    assert!(!nacked.load(std::sync::atomic::Ordering::SeqCst), "Message should not be nacked");
}
...
async fn test_ack_nack_middleware_auto_nack() {
    ...
    assert!(!acked.load(std::sync::atomic::Ordering::SeqCst), "Failed message should not be acked");
    assert!(nacked.load(std::sync::atomic::Ordering::SeqCst), "Failed message should be nacked");
    assert!(requeued.load(std::sync::atomic::Ordering::SeqCst), "Failed message should be requeued by default");
}
...
async fn test_circuit_breaker_middleware() {
    ...
    for i in 0..3 {
        let _ = pool.dispatch(create_test_message(&format!("msg-{}", i))).await;
    }
    ...
    assert!(processed >= 3, "At least initial messages should be processed");
}
...
async fn test_protected_api_scenario() {
    ...
    let api_worker = Arc::new(
        TestWorker::new("api-processor")
    ...
    for i in 0..30 {
        let _ = pool.dispatch(create_test_message(&format!("api-request-{:02}", i))).await;
    }
    
    // Wait for processing
    tokio::time::sleep(Duration::from_millis(1000)).await;
    
    let processed = api_worker.process_count.load(std::sync::atomic::Ordering::SeqCst);
    
    // Should have processed some but not all (rate limited)
    assert!(processed > 0, "Should have processed some requests");
    assert!(processed < 30, "Rate limiting should prevent all from completing immediately");
}
...
async fn test_multiple_same_middleware() {
    ...
    wait_for_condition(
        || worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 5,
        Duration::from_secs(2)
    ).await;
...
async fn test_middleware_execution_order() {
    ...
    assert_eq!(order[1], "second", "Second middleware should execute second");
...
async fn test_payment_processor_with_circuit_breaker() {
    ...
    wait_for_condition(
        || payment_worker.process_count.load(std::sync::atomic::Ordering::SeqCst) >= 10,
        Duration::from_secs(5)
    ).await;
    
    assert_eq!(payment_worker.process_count.load(std::sync::atomic::Ordering::SeqCst), 10);
}
...
async fn test_email_sender_with_rate_limit_and_tracing() {
    ...
    // Send 20 emails
    for i in 0..20 {
        let _ = pool.dispatch(create_test_message(&format!("email-{:02}", i))).await;
    }
    ...
    let processed = email_worker.process_count.load(std::sync::atomic::Ordering::SeqCst);
