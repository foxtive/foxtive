...
async fn test_initial_delay() {
    ...
    assert!(
        time_to_first_run >= Duration::from_millis(450),
        "First run was too early: {:?}",
        time_to_first_run
    );
    assert!(
        elapsed >= Duration::from_millis(950),
        "Total elapsed time too short: {:?}",
        elapsed
    );
}
...
async fn test_initial_delay_with_jitter() {
    ...
    assert!(
        time_to_first_run <= Duration::from_millis(800),
        "First run was too late (jitter may have been excessive): {:?}",
        time_to_first_run
    );
}
...
async fn test_rate_limiting_restart_interval() {
    ...
    assert!(
        interval_1 >= Duration::from_millis(450),
        "First restart interval too short: {:?} (should be >= 500ms)",
        interval_1
    );
    ...
    assert!(
        interval_2 >= Duration::from_millis(450),
        "Second restart interval too short: {:?} (should be >= 500ms)",
        interval_2
    );
}
...
async fn test_cron_task_with_very_frequent_schedule() {
    ...
    assert!(
        count >= 5,
        "Frequent cron should have run multiple times, got {}",
        count
    );
}
...
async fn test_combined_initial_delay_and_restart_backoff() {
    ...
    assert!(
        first_run_delay >= Duration::from_millis(180),
        "First run should respect initial delay: {:?}",
        first_run_delay
    );
}
