...
async fn test_multiple_groups_overlap() {
    ...
    let supervisor = Supervisor::new()
        .add(OverlapTask { id: "t1", group: Some("group-a") })
        .add(OverlapTask { id: "t2", group: Some("group-a") })
        .add(OverlapTask { id: "t3", group: Some("group-b") })
        .add(OverlapTask { id: "t4", group: Some("group-b") })
        .add(OverlapTask { id: "t5", group: None }); // No group
...
async fn test_group_health_all_healthy() {
    ...
    let supervisor = Supervisor::new()
        .add(HealthyTask { id: "h1", group: "all-healthy" })
        .add(HealthyTask { id: "h2", group: "all-healthy" })
        .add(HealthyTask { id: "h3", group: "all-healthy" });
...
async fn test_group_health_mixed_statuses() {
    ...
    let supervisor = Supervisor::new()
        ...
        .add(MixedHealthTask { 
            id: "degraded", 
            group: "mixed",
            status: HealthStatus::Degraded { reason: "Slow response".to_string() } 
        })
        .add(MixedHealthTask { 
            id: "unhealthy", 
            group: "mixed",
            status: HealthStatus::Unhealthy { reason: "Connection failed".to_string() } 
        });
...
async fn test_many_conditional_dependencies() {
    ...
    let supervisor = Supervisor::new()
        .add(ManyCondTask { id: "dep-1", setup_order: setup_order.clone() })
        .add(ManyCondTask { id: "dep-2", setup_order: setup_order.clone() })
        .add(ManyCondTask { id: "dep-3", setup_order: setup_order.clone() })
        .add(ManyCondTask { id: "dep-4", setup_order: setup_order.clone() })  // Won't be required
        .add(ManyCondTask { id: "main", setup_order: setup_order.clone() });
...
async fn test_group_with_conditional_deps() {
    ...
    let supervisor = Supervisor::new()
        .add(GroupCondTask { id: "opt-dep", group: "my-group", has_cond_dep: false })
        .add(GroupCondTask { id: "main-task", group: "my-group", has_cond_dep: true });
...
async fn test_rapid_group_stop_start() {
    ...
    let supervisor = Supervisor::new()
        .add(RapidTask { id: "r1", group: "rapid", count: count.clone() })
        .add(RapidTask { id: "r2", group: "rapid", count: count.clone() });
