...
async fn test_nested_hierarchy() {
    ...
    let hierarchy = SupervisorHierarchy::new("root").add_child("parent", parent_supervisor);
    ...
    assert!(
        ...
        "Nested task should have started"
    );
...
async fn test_deep_nested_hierarchy() {
    ...
    let level4 = Supervisor::new().add(SimpleTask {
        id: "level-4",
        started: started.clone(),
    });

    let level3 = Supervisor::new().add(SimpleTask {
        id: "level-3",
        started: started.clone(),
    });

    let level2 = Supervisor::new().add(SimpleTask {
        id: "level-2",
        started: started.clone(),
    });

    let level1 = Supervisor::new().add(SimpleTask {
        id: "level-1",
        started: started.clone(),
    });
...
async fn test_hierarchy_shutdown_order() {
    ...
    let child1 = Supervisor::new().add(ShutdownOrderTask {
        id: "child1-task",
        order: shutdown_order.clone(),
    });

    let child2 = Supervisor::new().add(ShutdownOrderTask {
        id: "child2-task",
        order: shutdown_order.clone(),
    });
...
async fn test_hierarchy_with_failing_task() {
    ...
    let failing_supervisor = Supervisor::new().add(FailingTask {
        id: "failing",
        attempts: attempts.clone(),
    });

    let working_supervisor = Supervisor::new().add(WorkingTask { id: "working" });
