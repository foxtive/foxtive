...
async fn test_hot_reload_restart_policy() {
    ...
    runtime
        .update_restart_policy(id, RestartPolicy::Never)
        .await
        .unwrap();
...
async fn test_hot_reload_backoff_strategy() {
    ...
    runtime
        .update_backoff_strategy(id, new_strategy.clone())
        .await
        .unwrap();
...
async fn test_config_change_events() {
    ...
    runtime
        .update_restart_policy(id, RestartPolicy::Always)
        .await
        .unwrap();
    ...
    assert!(
        received_event,
        "Should have received TaskConfigUpdated event"
    );
    ...
    if let Ok(SupervisorEvent::TaskConfigUpdated { field, .. }) = event_rx.try_recv() {
        assert_eq!(field, "backoff_strategy");
        received_second_event = true;
    }

    assert!(
        received_second_event,
        "Should have received second TaskConfigUpdated event"
    );
...
async fn test_validation_rejects_invalid_strategies() {
    ...
    assert!(
        result.is_err(),
        "Should reject invalid exponential strategy"
    );
    ...
    assert!(
        result.is_err(),
        "Should reject excessively long fixed delay"
    );
...
async fn test_update_unknown_task_returns_error() {
    ...
    let result = runtime
        .update_restart_policy("non-existent", RestartPolicy::Always)
        .await;
...
async fn test_concurrent_config_updates() {
    ...
    runtime
        ...
        .unwrap();
    runtime
        .update_backoff_strategy(id, BackoffStrategy::Fixed(Duration::from_millis(50)))
        .await
        .unwrap();
    runtime.set_task_enabled(id, false).await.unwrap();
...
async fn test_config_persistence_across_reads() {
    ...
    // Update config
    runtime
        .update_restart_policy(id, RestartPolicy::MaxAttempts(10))
        .await
        .unwrap();
    runtime
        .update_backoff_strategy(
...
async fn test_hot_reload_multiple_tasks() {
    ...
    for id in &task_ids {
        runtime
            ...
            .unwrap();
        runtime
            .update_backoff_strategy(id, BackoffStrategy::Fixed(Duration::from_millis(20)))
            .await
            .unwrap();
    }
...
async fn test_disabled_task_no_restart() {
    ...
    runtime
        .set_task_enabled("disabled-test", false)
        .await
        .unwrap();
...
async fn test_reenable_disabled_task() {
    ...
    // Disable the task
    runtime
        .set_task_enabled("reenable-test", false)
        .await
        .unwrap();
    assert!(!runtime.is_task_enabled("reenable-test").await);
    ...
    // Re-enable the task
    runtime
        .set_task_enabled("reenable-test", true)
        .await
        .unwrap();
    assert!(runtime.is_task_enabled("reenable-test").await);
...
async fn test_update_during_execution() {
    ...
    runtime
        .update_restart_policy("during-exec-test", RestartPolicy::Never)
        .await
        .unwrap();
