...
async fn test_pool_load_distribution() {
    ...
    assert_eq!(
        runs, pool_size,
        "Each worker in the pool should have run once"
    );
}
...
async fn test_pool_integration_with_groups() {
    ...
    assert_eq!(
        group_tasks.len(),
        4,
        "All pool workers should be in the same group"
    );
...
async fn test_pool_concurrent_start_stop() {
    ...
    for i in 0..10 {
        let ids = [
            "conc-0", "conc-1", "conc-2", "conc-3", "conc-4", "conc-5", "conc-6", "conc-7",
            "conc-8", "conc-9",
        ];
    ...
    assert_eq!(
        ...
        10,
        "All tasks should shutdown cleanly"
    );
}
