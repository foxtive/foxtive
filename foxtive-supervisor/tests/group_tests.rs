...
async fn test_task_groups_start_stop() {
    ...
    let supervisor = Supervisor::new()
        .add(GroupTask { id: "task-a1", group: "group-a", run_count: count_a.clone() })
        .add(GroupTask { id: "task-a2", group: "group-a", run_count: count_a.clone() })
        .add(GroupTask { id: "task-b1", group: "group-b", run_count: count_b.clone() });
    ...
    assert_eq!(count_a.load(Ordering::SeqCst), 2, "Both group-a tasks should be running");
    assert_eq!(count_b.load(Ordering::SeqCst), 1, "Group-b task should be running");
    ...
    assert_eq!(count_b.load(Ordering::SeqCst), 1, "Group-b should still be running");
...
async fn test_list_group_tasks() {
    ...
    let supervisor = Supervisor::new()
        .add(SimpleGroupTask { id: "db-conn", group: "database" })
        .add(SimpleGroupTask { id: "db-pool", group: "database" })
        .add(SimpleGroupTask { id: "api-server", group: "api" });
...
async fn test_restart_group() {
    ...
    let supervisor = Supervisor::new()
        .add(RestartableGroupTask { id: "worker-1", group: "workers" })
        .add(RestartableGroupTask { id: "worker-2", group: "workers" });
...
async fn test_group_health_aggregation() {
    ...
    let supervisor = Supervisor::new()
        .add(HealthTask { id: "healthy-1", group: "group-a", status: HealthStatus::Healthy })
        .add(HealthTask { id: "healthy-2", group: "group-a", status: HealthStatus::Healthy })
        .add(HealthTask { id: "degraded-1", group: "group-b", status: HealthStatus::Degraded { reason: "Test degradation".to_string() } })
        .add(HealthTask { id: "healthy-3", group: "group-b", status: HealthStatus::Healthy })
        .add(HealthTask { id: "unhealthy-1", group: "group-c", status: HealthStatus::Unhealthy { reason: "Test failure".to_string() } })
        .add(HealthTask { id: "healthy-4", group: "group-c", status: HealthStatus::Healthy });
