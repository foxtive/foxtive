...
impl MockTask {
    ...
    pub async fn with_policy(&mut self, policy: RestartPolicy) -> &mut Self {
        {
    ...
    pub async fn with_backoff(&mut self, backoff: BackoffStrategy) -> &mut Self {
        {
...
impl SupervisedTask for MockTask {
    ...
    fn restart_policy(&self) -> RestartPolicy {
        if let Ok(p) = self.restart_policy.try_lock() {
            p.clone()
        } else {
    ...
    fn backoff_strategy(&self) -> BackoffStrategy {
        if let Ok(b) = self.backoff.try_lock() {
            b.clone()
        } else {
...
impl TestHarness {
    ...
    pub fn add_mock(&mut self, id: &'static str) -> MockTask {
        ...
        if let Some(s) = self.supervisor.take() {
            self.supervisor = Some(s.add(task.clone()));
        }
        task
    }
...
impl<'a> TaskAssertions<'a> {
    ...
    pub fn new(runtime: &'a TaskRuntime) -> Self {
        Self { runtime }
    }
