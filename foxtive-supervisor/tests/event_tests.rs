...
async fn test_supervisor_events() {
    ...
    let listener = Arc::new(TestEventListener { events: events.clone() });
    ...
    assert!(captured.iter().any(|e| matches!(e, SupervisorEvent::TaskRegistered { id, .. } if id == "event_task")));
    assert!(captured.iter().any(|e| matches!(e, SupervisorEvent::TaskStarted { id, .. } if id == "event_task")));
    assert!(captured.iter().any(|e| matches!(e, SupervisorEvent::TaskFinished { id, .. } if id == "event_task")));
}
...
async fn test_shutdown_events() {
    ...
    let captured = events.lock().await;
    assert!(captured.iter().any(|e| matches!(e, SupervisorEvent::SupervisorShutdownStarted)));
    assert!(captured.iter().any(|e| matches!(e, SupervisorEvent::SupervisorShutdownCompleted)));
}
