...
mod tests {
    use foxtive_supervisor::distributed::{CoordinationBackend, CoordinationConfig, RedisCoordination};
    use std::sync::Arc;
    ...
    async fn test_leader_election_single_instance() {
        ...
        // Become leader
        let became_leader = backend.try_become_leader("leader-candidate", 10).await.unwrap();
    ...
    async fn test_heartbeat_expiration() {
        ...
        assert!(backend.is_instance_alive("expiring-instance").await.unwrap());
        
        // Wait for expiration
        tokio::time::sleep(Duration::from_secs(2)).await;
        
        // Instance should now be dead
        let is_alive = backend.is_instance_alive("expiring-instance").await.unwrap();
    ...
    async fn test_leader_lease_expiration() {
        ...
        assert!(became_leader, "New instance should become leader after old lease expires");
    }
    ...
    async fn test_coordination_manager() {
        ...
        let backend: Arc<dyn foxtive_supervisor::distributed::CoordinationBackend> = Arc::new(create_backend("manager-test").await.unwrap());
